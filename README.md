...
`os.system(command)` - Выполнить команду в командной оболочке

`os.name()` - Имя операционной системы ('posix', 'nt', 'java'). 
...
`stat()` - получения подробной информации (метаданных) о файле или директории по указанному пути
```python
import os
...
```
w Только для записи. Создаст новый файл, если не найдет с указанным именем., w+ Для чтения и записи. Создаст новый файл для записи, если не найдет с указанным именем., а r — reading (чтение). Больше о режимах открытия можно почитать https://pythonru.com/osnovy/fajly-v-python-vvod-vyvod#-open
